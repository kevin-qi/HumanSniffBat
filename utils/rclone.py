...
def check(SOURCE_PATH, SERVER_PATH, LOG_DIR=None):
    ...
    res = subprocess.run(['rclone', 'check', SOURCE_PATH, SERVER_PATH, '--size-only', '--one-way'])
...
def copy(SOURCE_PATH, SERVER_PATH):
    currentDT = datetime.datetime.now()
    log_time = currentDT.strftime("%y%m%d_%H_%M_%S")
    #LOG_PATH_COPY = os.path.join(LOG_DIR, 'COPY_{}.log'.format(log_time))

    res = subprocess.run(['rclone', 'copy', SOURCE_PATH, SERVER_PATH,'--size-only', '--transfers=16', '--checkers=8', '-P'])
    print(res)
    if(res.returncode):
