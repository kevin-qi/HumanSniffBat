...
def preprocess_raw_video(room_name, session_name, config):
    ...
    # Set paths
    data_path = '../data/raw/{}/{}'.format(room_name, session_name)
    ...
    for camera in camera_paths.keys():
        if(camera in config[room_name]['cameras'].keys()):
            ...
            for (width,height) in zip(crop_w,crop_h):
                ...
                video_out[(width,height)] = cv2.VideoWriter(os.path.join(out_dir,'{}x{}.mp4'.format(width, height)), fourcc, fps, (width, height))
            ...
            for i in range(num_frames):
                ...
                for (width,height) in zip(crop_w,crop_h):
                    ...
                    video_out[(width,height)].write(cropped_frame)
            ...
            for (width,height) in zip(crop_w,crop_h):
                width = min(width,transformed_img.shape[1]) # Maximum width
                height = min(height,transformed_img.shape[0]) # Maximum height
                video_out[(width,height)].release()
            cap.release()
